      ...
      {#each filteredProjects as project (project.id)}
        <div class="scene">
          ...
          <div 
            ...
            use:tilt={{ max: 14, scale: 1.03 }}
            class="group relative flex flex-col justify-between p-6 bg-[--color-surface] border border-[--color-border] rounded-xl hover:shadow-md hover:border-l-[3px] hover:border-l-[--color-accent] card-3d"
          >
            <div>
              ...
              <div class="mb-4">
                ...
                <div class="flex flex-wrap gap-1.5 overflow-hidden max-h-[52px]">
                  {#each project.techStack.slice(0, 5) as tech}
                    <span class="px-2 py-0.5 rounded-xs text-[10px] font-mono bg-[--color-bg] border border-[--color-border] text-[--color-muted] flex items-center gap-1">
...
<style>
  ...
  .card-3d {
    ...
    /* Transition only specific non-transform properties so we do not conflict with tilt.ts dynamic transitions */
    transition: border-color 350ms ease, border-left-width 350ms ease, box-shadow 350ms ease, background-color 350ms ease;
  }
</style>
